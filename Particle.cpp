...
Particle::Particle(Point boundary, double speedLimit, Function *function) {
...
void Particle::initializePosition() {
    //todo fci pro random generator plis
...
void Particle::receiveBestNeighbourResult(Point position, double fitness) {
...
void Particle::calculateNewVelocity() {
    ...
    velocity.x = w*velocity.x + r_p*(bestGlobalPosition.x - position.x) + r_g*(bestNeighbourPosition.x - position.x);
    velocity.y = w*velocity.y + r_p*(bestGlobalPosition.y - position.y) + r_g*(bestNeighbourPosition.y - position.y);
